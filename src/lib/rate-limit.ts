...
export function getRateLimiter(identifier: string, limit: number = 10, window: number = 60): Ratelimit {
...
export async function rateLimitRequest(
  ...
  try {
    ...
    const { success, limit: rateLimit, reset, remaining } = await limiter.limit(`${ip}`);
