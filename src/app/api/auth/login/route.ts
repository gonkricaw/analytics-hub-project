...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResponse = await rateLimitRequest(request, 'auth:login', 5, 60);
    ...
    if (user?.is_ip_blocked) {
      ...
      return NextResponse.json(
        { error: 'This account has been temporarily locked due to too many failed login attempts.' },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({ success: true, callbackUrl: callbackUrl || '/home' });
  } catch (error) {
