...
const defaultConfigs = {
  ...
  'system_maintenance_message': 'The system is currently undergoing maintenance. Please try again later.',
};
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const missingKeys = Object.keys(defaultConfigs).filter(key => !existingKeys.includes(key));
    ...
    if (missingKeys.length > 0) {
      ...
      return NextResponse.json(await prisma.idnbi_SystemConfiguration.findMany());
    }
  ...
  try {
    ...
    if (!Array.isArray(configs)) {
      return NextResponse.json(
        { error: 'Invalid request format. "configs" should be an array of key-value pairs.' },
        { status: 400 }
      );
    }
