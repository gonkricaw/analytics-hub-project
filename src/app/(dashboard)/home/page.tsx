...
export default function HomePage() {
  ...
  const [systemConfig, setSystemConfig] = useState<SystemConfig>({
    marquee_text: 'Welcome to Indonet Analytics Hub - Loading system announcements...',
    jumbotron_images: [],
    ...
    const fetchSystemConfig = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const configObj = data.reduce((acc: Record<string, unknown>, item: { key: string, value: string }) => {
            try {
          ...
          setSystemConfig({
            marquee_text: configObj.marquee_text || 'Welcome to Indonet Analytics Hub',
    ...
    const fetchAnalyticsData = async () => {
      try {
        ...
        setAnalyticsData({
          ...
          recentNotifications: [
            { title: 'System maintenance scheduled', date: new Date(Date.now() - 3600000).toISOString() },
            { title: 'New feature available', date: new Date(Date.now() - 86400000).toISOString() },
            { title: 'Welcome to the platform', date: new Date(Date.now() - 172800000).toISOString() },
          ]
        });
  ...
  const jumbotronSlides = systemConfig.jumbotron_images.length > 0
    ? systemConfig.jumbotron_images.map((image, index) => (
        <div key={index} className="w-full h-64 relative rounded-xl overflow-hidden">
    ...
    : [
        ...
        <div key="slide1" className="w-full h-64 bg-gradient-to-r from-blue-600 to-indigo-700 flex items-center justify-center rounded-xl">
          <h2 className="text-2xl font-bold text-white">Welcome to {systemConfig.app_name}</h2>
        </div>,
        <div key="slide2" className="w-full h-64 bg-gradient-to-r from-purple-600 to-pink-700 flex items-center justify-center rounded-xl">
          <h2 className="text-2xl font-bold text-white">Powerful Analytics Platform</h2>
        </div>
      ];
        ...
        <Card className="md:w-2/3">
          ...
          <CardContent>
            <p className="text-muted-foreground">
              {session?.user ? `You are logged in as: ${session.user.email}` : 'Loading session information...'}
            </p>
            {session?.user?.role && (
      ...
      <Card>
        <CardContent className="p-0 overflow-hidden">
          <div className="bg-primary p-3">
            <Marquee pauseOnHover speed={30} className="text-primary-foreground font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-2">Users online now</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold">{analyticsData.loginStats}</div>
              <p className="text-xs text-muted-foreground mt-2">Logins in the past 24 hours</p>
            </CardContent>
          </Card>
